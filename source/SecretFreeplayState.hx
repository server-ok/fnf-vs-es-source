...
{
	public static var songs:Array<SecretSongMetadata> = [];
	...
	{
		clean();
		var initSonglist = CoolUtil.coolTextFile(Paths.txt('data/secretsongs'));
		...
		for (i in 0...initSonglist.length)
		{
			...
			var meta = new SecretSongMetadata(data[0], Std.parseInt(data[2]), data[1]);
			...
			if (diffsThatExist.contains("Easy"))
				SecretFreeplayState.loadDiff(0,format,meta.songName,diffs);
			if (diffsThatExist.contains("Normal"))
				SecretFreeplayState.loadDiff(1,format,meta.songName,diffs);
			if (diffsThatExist.contains("Hard"))
				SecretFreeplayState.loadDiff(2,format,meta.songName,diffs);
			...
			SecretFreeplayState.songData.set(meta.songName,diffs);
			...
			if (FileSystem.isDirectory("assets/sm/" + i))
			{
				...
				for (file in FileSystem.readDirectory("assets/sm/" + i))
				{
					...
					if (file.endsWith(".sm") && !FileSystem.exists("assets/sm/" + i + "/converted.json"))
					{
						...
						var meta = new SecretSongMetadata(file.header.TITLE, 0, "sm",file,"assets/sm/" + i);
						songs.push(meta);
	...
	{
		songs.push(new SecretSongMetadata(songName, weekNum, songCharacter));
	}
